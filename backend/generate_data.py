...
def generate_data():
    ...
    with conn.cursor() as cur:
        ...
        for i in range(product_count):
            product_rows.append((f"Product {i}", "Electronics" if i % 2 == 0 else "Furniture", 10 + i, 100 + i, start_dt, end_dt))
        insert_rows(cur, "products", ["product_name", "category", "price", "stock_quantity", "created_at", "updated_at"], product_rows)

        order_rows = []
        for i in range(order_count):
            order_rows.append(((i % 10) + 1, start_dt + timedelta(days=(i % days)), 20 + i, "Completed" if i % 2 == 0 else "Pending", start_dt, end_dt))
        ...
        for i in range(payment_count):
            payment_rows.append((i + 1, "Card" if i % 2 == 0 else "Cash", "Completed" if i % 2 == 0 else "Pending", 20 + i, start_dt + timedelta(days=i % days), start_dt, end_dt))
